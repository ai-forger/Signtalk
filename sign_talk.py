    ...
    def _on_button_hover(self, button, is_hovered):
        ...
        if self.theme_mode == 'dark':
            if button == self.space_btn:
                button.config(bg='#dc2626' if is_hovered else '#ef4444')
            elif button == self.clear_btn:
                button.config(bg='#d97706' if is_hovered else '#f59e0b')
            elif button == self.speech_btn:
                button.config(bg='#16a34a' if is_hovered else '#22c55e')
            elif button == self.speak_btn:
                button.config(bg='#2563eb' if is_hovered else '#3b82f6')
            elif button == self.backspace_btn: # New backspace button
                button.config(bg='#6b21a8' if is_hovered else '#8b5cf6')
            elif button in [self.b1, self.b2, self.b3, self.b4]:
                button.config(bg='#6d28d9' if is_hovered else '#8b5cf6')
        else: # Light mode
            if button == self.space_btn:
                button.config(bg='#b91c1c' if is_hovered else '#ef4444')
            elif button == self.clear_btn:
                button.config(bg='#b45309' if is_hovered else '#f59e0b')
            elif button == self.speech_btn:
                button.config(bg='#15803d' if is_hovered else '#16a34a')
            elif button == self.speak_btn:
                button.config(bg='#1d4ed8' if is_hovered else '#2563eb')
            elif button == self.backspace_btn: # New backspace button
                button.config(bg='#5b21b6' if is_hovered else '#7c3aed')
            elif button in [self.b1, self.b2, self.b3, self.b4]:
                button.config(bg='#6d28d9' if is_hovered else '#7c3aed')
